...
However, we can see that the variables describing a single person `p1` are stored separately. Imagine having 100 people, we will have 300 lines of code storing only the properties of every person. That's very repetitive and inefficient!
...
* `from_what` = specifying the reference point
    * 0 = beginning of file
